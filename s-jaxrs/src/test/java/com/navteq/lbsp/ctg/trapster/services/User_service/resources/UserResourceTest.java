...
public class UserResourceTest {
	...
	//private static final String SRV_URI = "http://localhost:3001/User-service-example";
	...
	public void testGETbyDISPLAYNAME() throws Exception {
		...
		String dispName="testName";
		
        postRequest(createPostRequestWithDisplayName(SRV_URI, dispName));
		
		// TODO build and execute request
		String fullUrl=SRV_URI+"/query";//?displayName="+dispName;
		this.LOGGER.debug(fullUrl);
		ClientRequest request = new ClientRequest(fullUrl);
		request.accept(MediaType.APPLICATION_XML);
		request.queryParameter("displayName", dispName);
	
		ClientResponse<TrapsterType> response = request
		.get(TrapsterType.class);
		
		TrapsterType trapsterType = (TrapsterType) response.getEntity();
		Assert.assertEquals(dispName, trapsterType.getResponse().getData().getDisplayname());
	...
	public void testPOST() throws Exception {
		...
		String uname="postUser";
		ClientRequest request=createPostRequestWithDisplayName(SRV_URI, uname);
		
		postRequest(request);
		
		// TODO check database for new record
		Udb udb=this.udbDao.getByNameOrUname(uname);
		Assert.assertEquals(uname, udb.getUname());
	...
	public void testGet() throws Exception {
		...
		postRequest(createPostRequestWithDisplayName(SRV_URI, "testGet"));
		
		// build and execute request
		ClientRequest request = new ClientRequest(SRV_URI+"/query");
	
	
		request.accept(MediaType.APPLICATION_XML);
		request.queryParameter("id", this.testUserId);
	...
	public void testDelete() throws Exception {
		...
		Udb user=this.udbDao.getById(new Long(testUserId));
		Assert.assertNull(user);
		
		postRequest(createPostRequestWithDisplayName(SRV_URI, "testDelete"));
		
		user=this.udbDao.getById(new Long(testUserId));
		Assert.assertNotNull(user);
		 
		ClientRequest request = new ClientRequest(SRV_URI + "/delete/" + testUserId);
	...
	public void deleteDemoUser(){
		this.udbDao.removeById(1L);

	}
	
	private TrapsterType getTrapsterTypeWithDispName(String dispName){
		TrapsterType tt=new TrapsterType();
		TrapsterRequestType reqType =new TrapsterRequestType();
		tt.setRequest(reqType);
		...
		if(dispName !=null && dispName.length()>0)
			...
			disp=this.testPayload.getRequest().getParams().getDisplayname();
		pt.setDisplayname(disp);
		tt.getRequest().setParams(pt);
		return tt;
	}

	
	private ClientRequest createPostRequestWithDisplayName(String uri,String dispName){
		// build and execute request
				ClientRequest request = new ClientRequest(uri);
				request.accept(MediaType.APPLICATION_XML);

				TrapsterType tt=this.getTrapsterTypeWithDispName(dispName);
				request.body(MediaType.APPLICATION_XML, tt);
				
				return request;
				
	}
	private TrapsterType postRequest(ClientRequest request)throws Exception{
		...
		TrapsterType trapsterType = (TrapsterType) response.getEntity();
		this.LOGGER.debug("back from post!!!!!");
		this.LOGGER.debug(trapsterType.getResponse().getData().getId());
		response.releaseConnection();
		return trapsterType;
	}
